...
csv_to_sqlite(csv_file = "flights.csv",
              sqlite_file = "/Users/janewillden/combined.sqlite",
              table_name = "flights")
...
combined_flight <- DBI::dbConnect(RSQLite::SQLite(),
                                  "/Users/janewillden/Downloads/combined.sqlite")
DBI::dbListTables(combined_flight)
...
combined_flight <- dbConnect(SQLite(), 
                             "/Users/janewillden/Downloads/combined.sqlite")
dbGetQuery(combined_flight, "PRAGMA table_info('flights');")
```
...
combined_flight <- dbConnect(
  SQLite(), "/Users/janewillden/Downloads/combined.sqlite")

# 1. Average distance of all flights
dbGetQuery(
  combined_flight, "SELECT AVG(DISTANCE) AS avg_distance FROM flights;")

# 2. Minimum flight distance
dbGetQuery(combined_flight, "SELECT MIN(DISTANCE) AS min_distance FROM flights;")

# 3. Maximum flight distance
dbGetQuery(combined_flight, "SELECT MAX(DISTANCE) AS max_distance FROM flights;")

# 4. Airports for the minimum distance flight
dbGetQuery(combined_flight, "SELECT ORIGIN_AIRPORT, DESTINATION_AIRPORT, DISTANCE FROM flights
           WHERE DISTANCE = (SELECT MIN(DISTANCE) FROM flights);")

# 5. Airports for the maximum distance flight
dbGetQuery(combined_flight, "SELECT DISTINCT ORIGIN_AIRPORT, DESTINATION_AIRPORT, DISTANCE FROM flights
           WHERE DISTANCE = (SELECT MAX(DISTANCE) FROM flights);")

# 6. Maximum distance, excluding numeric airport codes
dbGetQuery(combined_flight, "SELECT DISTINCT ORIGIN_AIRPORT, DESTINATION_AIRPORT, DISTANCE FROM flights
           WHERE DISTANCE = (SELECT MAX(DISTANCE) FROM flights)
              AND ORIGIN_AIRPORT NOT LIKE '1%'
              AND DESTINATION_AIRPORT NOT LIKE '1%';")

# 7. Minimum distance flight -- airport names
dbGetQuery(combined_flight, "SELECT
...
# 8. Maximum distance flight -- airport names
dbGetQuery(combined_flight, "SELECT DISTINCT
              a1.AIRPORT AS origin_airport,
              a2.AIRPORT AS dest_airport,
              f.DISTANCE
            FROM flights f
...
combined_flight <- dbConnect(SQLite(), "/Users/janewillden/Downloads/combined.sqlite")

delay_data <- dbGetQuery(combined_flight, "SELECT
                            f.ARRIVAL_DELAY,
...
```{sql avg_delay_airline, connection=combined_flight, output.var="avg_delay_airline"}
...
ggplot(avg_delay_airline,
       aes(x = airline_name, y = avg_arrival_delay)) +
  geom_col() +
  labs(
    x = "Airline",
...
flights_tbl <- tbl(combined_flight, "flights")

airlines_tbl <- tbl(combined_flight, "airlines")
...
```{r}
ggplot(summary_stats, aes(x = AIRLINE, y = sd_elapsed_time)) +
  geom_col() +
  labs(x = "Airline",
      y = "Standard Deviation of Elapsed Time (minutes)",
